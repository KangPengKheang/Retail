...
# Load logo
with open(RETAIL_LOGO_FILE, "rb") as f:
...
def get_service_account_info():
    ...
    st.error("Google Sheets credentials not found in Streamlit secrets or Kheang.json")
    return None
...
def load_users_from_sheets(_gc, sheet_id, worksheet_name=LOGIN_WORKSHEET_NAME):
...
def authenticate_simple_user(password):
    ...
    try:
        ...
        users_dict = load_users_from_sheets(gc, LOGIN_SHEET_ID, LOGIN_WORKSHEET_NAME)
...
def debug_users_sheet():
    ...
    try:
        sheet = gc.open_by_key(LOGIN_SHEET_ID)
        worksheet = sheet.worksheet(LOGIN_WORKSHEET_NAME)
        ...
        users = load_users_from_sheets(gc, LOGIN_SHEET_ID, LOGIN_WORKSHEET_NAME)
...
def login_form():
    ...
    st.markdown(
        """
        <div class='retail-login-logo'>
            <img src='data:image/png;base64,{}' alt='Chip Mong Retail Logo'>
        </div>
        """.format(
            logo_data
        ),
    ...
    st.markdown(
        """
        <h3 style="color: #3f3f46; margin-bottom: 5px; font-size: 20px;">Market Visit Customer</h3>
        ...
        if submitted:
            if not password:
                ...
                if user_data:
                    ...
                    st.session_state.staff_id = user_data["username"]
...
def save_daily_plan_to_sheet(plan_data):
    try:
        ...
        sheet = gc.open_by_key(SUBMISSION_SHEET_ID)
        worksheet = sheet.worksheet(SUBMISSION_WORKSHEET_NAME)
...
def display_and_submit_plan():
    ...
    try:
        ...
        rm_name = st.session_state.get("user_data", {}).get("username", current_staff or "Unknown")
...
def get_rm_name_from_staff_id(_gc, staff_id):
    ...
    try:
        ...
        # Load users data
        users_data = load_users_from_sheets(_gc, LOGIN_SHEET_ID, LOGIN_WORKSHEET_NAME)
...
def debug_users_sheet_structure(_gc):
    ...
    try:
        ...
        worksheet = _gc.open_by_key(LOGIN_SHEET_ID).worksheet(LOGIN_WORKSHEET_NAME)
...
def debug_rm_lookup():
    ...
    if current_staff:
        ...
        try:
            users_data = load_users_from_sheets(gc, LOGIN_SHEET_ID, LOGIN_WORKSHEET_NAME)
    ...
    with header_col2:
        st.markdown(
            ...
            <div style="text-align: center; padding: 15px;">
                <h1 style="color: #3f3f46; margin: 0; font-size: 2.2rem; font-weight: 700;">
                    📍 Market Visit Customer
                </h1>
                <p style="color: #e83e8c; margin: 5px 0 0 0; font-size: 1.1rem; font-weight: 500;">
                    Chip Mong Retail
                </p>
            </div>
            """,
